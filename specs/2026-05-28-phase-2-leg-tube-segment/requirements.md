# Requirements — Phase 2: Leg Tube Segment + Tube Sleeve Connector
...
These two parts together replace the dowel-pin joining strategy with a tool-free, hardware-free threaded connection.
...
- No modelling of the assembled stack (that is Phase 12).
- No locking mechanism or anti-rotation feature in v1.
- No fillet/chamfer on the tube body ends unless it aids print quality.

## Context

At 170 mm body per segment, 6 segments + 5 sleeves = ~1070 mm per leg half; two half-legs joined = full ~1300 mm leg. Each sleeve weighs ~10 g printed; the connection is tool-free and re-usable. The thread geometry (M20-pitch-4) is reused by the X-frame hinge bracket and top bracket in later phases.
