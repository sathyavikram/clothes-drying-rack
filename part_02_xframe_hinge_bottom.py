...
def construct_hinge_bottom():
    ...
    f1 = App.Vector(inner_X_m,  0, -t_pitch * 0.45)
    f2 = App.Vector(tf_radius, 0, -t_pitch * 0.15)
    f3 = App.Vector(tf_radius, 0,  t_pitch * 0.15)
    f4 = App.Vector(inner_X_m,  0,  t_pitch * 0.45)
    ...
    sock_cutter_base = tf_core.fuse(tf_sweep)
    ...
    p_r_inner = p_radius - (p_pitch * 0.55)
    ...
    inner_X_p = p_r_inner - 0.5 * params.SCALE
    p_1 = App.Vector(inner_X_p,  0, -p_pitch * 0.45)
    p_2 = App.Vector(p_radius, 0, -p_pitch * 0.15)
    p_3 = App.Vector(p_radius, 0,  p_pitch * 0.15)
    p_4 = App.Vector(inner_X_p,  0,  p_pitch * 0.45)
    ...
    pivot_cutter_base = p_core.fuse(p_sweep)
    ...
    pivot_cutter = pivot_cutter.cut(bottom_trimmer)
    
    pivot_chamfer = Part.makeCone(p_radius+1.0, p_radius-1.0, 2.0, App.Vector(0,0,HUB_H-2.0))
    pivot_cutter = pivot_cutter.fuse(pivot_chamfer)

    # Cut pivot thread into arm
    shape_cut = body_base.cut(pivot_cutter)
